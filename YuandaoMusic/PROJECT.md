...
**原道音乐** 是一个 Android 本地 HiFi 音乐播放器。当前处于 **第一阶段：本地 WAV/FLAC 播放闭环验证**，并已提前补齐最小本地曲库搜索。
...
| **曲库** | 本地曲目搜索 | ✅ | 按歌曲名、艺术家、专辑名匹配；暂未加入排序和复杂筛选 |
| | 封面提取与缓存 | ✅ | 更多格式需要实机样本测试 |
| **播放** | WAV/FLAC 播放 | 🔶 | 播放闭环代码与自动化验证通过；当前改动后的设备实播待复验 |
...
| | 完整构建验证 | ✅ | 2026-09-04：单测、Debug APK、Lint 均通过 |
| | 模拟器实播验证 | ⬜ | 当前环境无法进行设备验证 |
...
### 6.1 设备运行时验证（当前环境暂无法完成）

本机质量闸门已完成；以下项目必须在 API 35 模拟器或真机上验证：

- [x] 完整构建：`.\gradlew.bat testDebugUnitTest assembleDebug lintDebug --stacktrace`
- [ ] 安装到 API 35 模拟器
...
- [x] 最小本地曲库搜索：按歌曲名、艺术家、专辑名匹配，结果可直接播放（设备视觉验证待完成）
- [x] 播放和扫描错误已转换为中文提示，并提供重新扫描、重新授权等操作建议（设备专项验证待完成）
- [ ] 播放详情页打磨：大封面、曲目信息、采样率/位深/格式、进度、歌词区域、播放模式切换
- [x] 队列项点击播放、移除队列项、清空队列（设备运行时验证待完成）
- [ ] 队列页验证：当前播放列表、下一首列表、随机队列顺序
- [x] Mini player 底部安全 padding：已加入 `safeDrawingPadding()`（设备视觉验证待完成）
...
1. 读取本文件，了解当前状态
2. 设备可用后安装到 API 35 模拟器或真机
3. 重新推入 WAV/FLAC 测试文件，验证扫描和实播
4. 重点观察前台服务修复是否彻底解决后台播放崩溃
5. 手测通知栏动作和系统媒体会话
6. 继续完善播放详情页和队列页
7. 每完成一个阶段，更新本文件对应 checklist 的状态
...
最新验证结果（2026-09-04）：`BUILD SUCCESSFUL`（`testDebugUnitTest assembleDebug lintDebug`）
